...
qui drop if year<2005 & year!=.
qui drop if year>2009 & year!=.
*
...
*drop short spells (unestable for estimation)
g between=endepi-begepi
drop if between!=. & between<11
...
qui drop if def_daily_wage<3 & def_daily_wage!=. & source==1
qui drop if def_daily_wage<13.433 & def_daily_wage!=. & form==1
qui drop if def_daily_wage<8.33 & def_daily_wage!=. & form==2
qui drop if def_daily_wage<5 & def_daily_wage!=. & form==3
qui drop if def_daily_wage>28.33 & def_daily_wage!=. & form==3
qui drop if def_daily_wage>100 & def_daily_wage!=. & source==2
qui drop if def_daily_wage<5 & def_daily_wage!=. & source==2
*break after 1999
*qui drop if def_daily_wage<14 & def_daily_wage!=. & source==1 & form==2

qui drop if def_daily_wage<1 & def_daily_wage!=. & source!=. & source>1
qui drop if def_daily_wage<1 & def_daily_wage!=. & source!=. & source>2
qui drop if def_daily_wage<5 & def_daily_wage!=. & source!=. & source>2
*qui replace def_daily_wage=1 if def_daily_wage<1 & def_daily_wage!=. & source!=. & source>2
...
qui replace def_daily_wage=1 if source==5
qui replace def_daily_wage=1 if source==4
qui replace def_daily_wage=1 if source==3
...
/*
hist def_daily_wage if source>1 & source<5
(bin=58, start=5, width=1.1611274)

. hist def_daily_wage if source>1 & source<3
(bin=55, start=5, width=1.2244616)

. hist def_daily_wage if source>2 & source<4
(bin=1, start=5, width=1)

. hist def_daily_wage if source>3 & source<5

. hist def_daily_wage if source>4 & source<6
(bin=1, start=5, width=1)
*/
qui g unemp_ben=1 if source==2
...
qui gen NE_month=1 if max_days_NE>max_days_month & max_days_month<=5
qui replace full_month=. if max_days_NE>max_days_month & max_days_month<=5
qui replace part_month=. if max_days_NE>max_days_month & max_days_month<=5
qui replace mini_month=. if max_days_NE>max_days_month & max_days_month<=5

qui replace full_month=1 if max_days_full>max_days_part & max_days_full>max_days_mini & full_month==. & mini_month==. & part_month==. & NE_month==.
qui replace part_month=1 if max_days_part>max_days_full & max_days_part>max_days_mini & full_month==. & mini_month==. & part_month==. & NE_month==.
qui replace mini_month=1 if max_days_mini>max_days_part & max_days_mini>max_days_mini & full_month==. & mini_month==. & part_month==. & NE_month==.

bysort persnr year month_begepi: replace counter_diff_estab_id=0 if NE_month==1
*some conc_month don't make the cut!
...
*& max_days_full+max_days_part+max_days_mini<65 
...
g mini_job=1 if days_mini>0 & days_mini!=.
replace mini_job=1 if days_NE>0 & days_NE!=.

bysort persnr year month_begepi: egen tot_monthly_earnings_FT=total(monthly_earnings) if days_full>0 & days_full!=.
bysort persnr year month_begepi: egen tot_monthly_earnings_CJ=total(monthly_earnings) if conc_month==1
bysort persnr year month_begepi: egen tot_monthly_earnings_PT=total(monthly_earnings) if days_part>0 & days_part!=.
bysort persnr year month_begepi: egen tot_monthly_earnings_MJ=total(monthly_earnings) if mini_job==1
bysort persnr year month_begepi: egen tot_monthly_earnings_NE=total(monthly_earnings) if days_NE>0 & days_NE!=.

*impute earnings to avoid middle of month changes:
bysort persnr year month_begepi: replace tot_monthly_earnings_FT=tot_monthly_earnings_FT*30/max_days_full if days_full>0 & days_full!=. & tot_monthly_earnings_FT<405
bysort persnr year month_begepi: replace tot_monthly_earnings_CJ=tot_monthly_earnings_CJ*60/(max_days_full+max_days_part+max_days_mini) if conc_month==1 & tot_monthly_earnings_CJ<405
bysort persnr year month_begepi: replace tot_monthly_earnings_PT=tot_monthly_earnings_PT*30/max_days_part if days_part>0 & days_part!=. & tot_monthly_earnings_PT<250
bysort persnr year month_begepi: replace tot_monthly_earnings_MJ=tot_monthly_earnings_MJ*30/max_days_mini if mini_job==1 & tot_monthly_earnings_MJ<150
bysort persnr year month_begepi: replace tot_monthly_earnings_NE=tot_monthly_earnings_NE*30/max_days_NE if days_NE>0 & days_NE!=. & tot_monthly_earnings_NE<150

bysort persnr year month_begepi: g  imputed_monthly_earnings=tot_monthly_earnings_FT if full_month==1
*bysort persnr year month_begepi: replace  imputed_monthly_earnings=tot_monthly_earnings_FT if conc_month==1 & max_days_full>=max_days_min & max_days_full>=max_days_part
*bysort persnr year month_begepi: replace  imputed_monthly_earnings=tot_monthly_earnings_PT if conc_month==1 & max_days_part>=max_days_mini & max_days_full<=max_days_part
*bysort persnr year month_begepi: replace  imputed_monthly_earnings=tot_monthly_earnings_MJ if conc_month==1 & max_days_part<=max_days_mini & max_days_mini>=max_days_full
bysort persnr year month_begepi: replace  imputed_monthly_earnings=tot_monthly_earnings_PT if part_month==1
bysort persnr year month_begepi: replace  imputed_monthly_earnings=tot_monthly_earnings_MJ if mini_month==1
bysort persnr year month_begepi: replace  imputed_monthly_earnings=tot_monthly_earnings_NE if NE_month==1
bysort persnr year month_begepi: replace  imputed_monthly_earnings=tot_monthly_earnings_CJ if conc_month==1
bysort persnr year month_begepi: egen max_imputed_monthly_earnings=max(imputed_monthly_earnings)
replace max_imputed_monthly_earnings=50 if max_imputed_monthly_earnings<50 & max_imputed_monthly_earnings!=.
/*
bysort persnr year month_begepi: egen tot_monthly_earnings=total(monthly_earnings)
bysort persnr year month_begepi: egen tot_real_monthly_earnings=total(monthly_earnings) if source>0 & source<3
bysort persnr year month_begepi: egen max_real_monthly_earnings=max(tot_real_monthly_earnings)
bysort persnr year month_begepi: g imputed_monthly_earnings=tot_monthly_earnings if tot_monthly_earnings==tot_real_monthly_earnings
bysort persnr year month_begepi: replace imputed_monthly_earnings=tot_monthly_earnings if max_real_monthly_earnings==.
bysort persnr year month_begepi: replace imputed_monthly_earnings=max_real_monthly_earnings if imputed_monthly_earnings==. & max_real_monthly_earnings!=.

bysort persnr year month_begepi: gen average_wage=imputed_monthly_earnings
bysort persnr year month_begepi: egen max_average_wage=max(average_wage)
*/
*
*
qui gen log_ave_wage=log(max_imputed_monthly_earnings)
*
replace unemp_ben=. if log_ave_wage<5 & log_ave_wage!=. & NE_month==1
*
*right tail stabilization for estimation, CJ:
g tc=1 if log_ave_wage>=9 & conc_month==1
local T=9
qui sum tc if conc_month==1
local F2=r(mean) 
_pctile log_ave_wage if log_ave_wage!=0 & conc_month==1, p(90)
local y9=r(r1)
local alpha9=ln((1-0.9)/(`F2'))/ln(`T'/`y9')

qui replace log_ave_wage=`T'/((1-runiform())^(1/`alpha9')) if tc==1 & conc_month==1

drop tc
*right tail stabilization for estimation, FT:
g tc=1 if log_ave_wage>=9 & full_month==1
local T=9
qui sum tc if full_month==1
local F2=r(mean) 
_pctile log_ave_wage if log_ave_wage!=0 & full_month==1, p(90)
local y9=r(r1)
local alpha9=ln((1-0.9)/(`F2'))/ln(`T'/`y9')

qui replace log_ave_wage=`T'/((1-runiform())^(1/`alpha9')) if tc==1 & full_month==1

drop tc
*right tail stabilization for estimation, PT:
g tc=1 if log_ave_wage>=8.5 & part_month==1
local T=8.5
qui sum tc if part_month==1
local F2=r(mean) 
_pctile log_ave_wage if log_ave_wage!=0 & part_month==1, p(90)
local y9=r(r1)
local alpha9=ln((1-0.9)/(`F2'))/ln(`T'/`y9')

qui replace log_ave_wage=`T'/((1-runiform())^(1/`alpha9')) if tc==1 & part_month==1

drop tc
*right tail stabilization for estimation, MJ:
g tc=1 if log_ave_wage>=8.5 & mini_month==1
local T=8.5
qui sum tc if mini_month==1
local F2=r(mean) 
_pctile log_ave_wage if log_ave_wage!=0 & mini_month==1, p(90)
local y9=r(r1)
local alpha9=ln((1-0.9)/(`F2'))/ln(`T'/`y9')

qui replace log_ave_wage=`T'/((1-runiform())^(1/`alpha9')) if tc==1 & mini_month==1

drop tc
*
*
*trim data to not drop so many individual observations?
*drop too low PT earnings (unestable for estimation!):
*bysort persnr: g too_low=1 if log_ave_wage!=. & log_ave_wage<5.5 & part_month==1
*bysort persnr: egen sum_too_low=total(too_low)
*drop if sum_too_low!=. & sum_too_low>0 
*drop too_low sum_too_low
*drop too low NE earnings (unestable for estimation!):
*bysort persnr: g too_low=1 if log_ave_wage!=. & log_ave_wage<5 & NE_month==1
*bysort persnr: egen sum_too_low=total(too_low)
*drop if sum_too_low!=. & sum_too_low>0 
*drop too_low sum_too_low
*drop too high NE earnings (unestable for estimation!):
*bysort persnr: g too_low=1 if log_ave_wage!=. & log_ave_wage>8 & NE_month==1
*bysort persnr: egen sum_too_low=total(too_low)
*drop if sum_too_low!=. & sum_too_low>0 
*drop too_low sum_too_low
*drop too low FT earnings (unestable for estimation!):
*bysort persnr: g too_low=1 if log_ave_wage!=. & log_ave_wage<6 & full_month==1
*bysort persnr: egen sum_too_low=total(too_low)
*drop if sum_too_low!=. & sum_too_low>0 
*drop too_low sum_too_low
*drop too high MJ earnings (unestable for estimation!):
*bysort persnr: g too_low=1 if log_ave_wage!=. & log_ave_wage>6.7 & mini_month==1
*bysort persnr: egen sum_too_low=total(too_low)
*drop if sum_too_low!=. & sum_too_low>0 
*drop too_low sum_too_low
*
*
order persnr* year month_begepi imputed_monthly* tot* max* log_ave_wage source
*
*drop tot_monthly_earnings 
drop def_daily_wage
drop monthly_earnings
...
***
*keep
drop begepi endepi counter_diff_estab_id sum_conc days_epi source counter_flow
...
order persnr* nobs nobs_year year month female year_birth educ age log_ave_wage full_month part_month mini_month NE_month conc_month unemp_ben

keep persnr* nobs nobs_year year month female year_birth educ age log_ave_wage full_month part_month mini_month NE_month conc_month FT* PT* MT* UT* CT* MJ NE CJ unemp_ben
...
g ne=1 if empstate==4
bysort persnr: egen tot_ne=total(ne)
drop if tot_ne==60

g info=1 if empstate==1 | empstate==2 | empstate==3 | empstate==5 | unemp_ben==1
bysort persnr: egen tot_info=total(info)
drop if tot_info<12
...
replace unemp_ben=0 if empstate==1 & unemp_ben==1
replace unemp_ben=0 if empstate==2 & unemp_ben==1
replace unemp_ben=0 if empstate==3 & unemp_ben==1
replace unemp_ben=0 if empstate==5 & unemp_ben==1

save "C:\Users\manuz\Desktop\RSIAB7514\EarningsDynamics&Institutions\Codes\Preliminars_Monthly_persnr_20052009.dta", replace
...
outsheet using "C:\Users\manuz\Desktop\RSIAB7514\EarningsDynamics&Institutions\Codes\Matlab\MonthlyIncome\2005-2009\data_persnr_males.csv", comma nolabel replace nonames

save "C:\Users\manuz\Desktop\RSIAB7514\EarningsDynamics&Institutions\Codes\Preliminars_Monthly_persnr_males_20052009.dta", replace
...
outsheet using "C:\Users\manuz\Desktop\RSIAB7514\EarningsDynamics&Institutions\Codes\Matlab\MonthlyIncome\2005-2009\data_persnr_females.csv", comma nolabel replace nonames

save "C:\Users\manuz\Desktop\RSIAB7514\EarningsDynamics&Institutions\Codes\Preliminars_Monthly_persnr_females_20052009.dta", replace

restore
*
/*
. hist log_ave_wage if empstate==4
(bin=57, start=3.9120231, width=.10452991)

. hist log_ave_wage if empstate==4 & unemp_ben==1
(bin=52, start=4.3174882, width=.08714158)

. hist log_ave_wage if empstate==4 & unemp_ben!=1
(bin=56, start=3.9120231, width=.10639651)
*/
