...
n_draws = 100; 
len=29;
estimates_draws_simplified = zeros(n_draws,len);
H=20000;
T=32;
col=T-1;
...
log_panel=sortrows(log_panel,[1 4]);%age year year_birth/coh
log_panel(1,end+1) = 1;
for i=2:length(log_panel)
if log_panel(i,1)==log_panel(i-1,1) && log_panel(i,4)==log_panel(i-1,4)
   log_panel(i,end)=0;
else
    log_panel(i,9)=1;    
end
...
data.M_ave=M_ave;
%
opts_fmincon = optimset('MaxFunEval',100000,'TolFun',1.0e-6,'TolX',1.0e-06,'Algorithm','sqp','Display','iter','DiffMinChange',1e-4);%,'UseParallel',true
estimates_draws_simplified(q,:)=fmincon(@(x) loss_simplified_se(x,data,H,T,randn_b,rand_shock_s,col,randn_a),para,[],[],[],[],lb,ub,[],opts_fmincon);
% 
toc
end
%delete(gcp)
%SE_simplified
SE_simplified=std(estimates_draws_simplified)
save('SE_simplified.mat','SE_simplified')
save('estimates_draws_simplified.mat','estimates_draws_simplified')
%
stars=para./(SE_simplified')