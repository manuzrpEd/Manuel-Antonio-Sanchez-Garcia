function count = accumarray_expand(data,index,fcn)
%performs function (fcn specified of the type @(x)f(x) or @(x){f(x)}) on 
%group data with individual index and returns column vector of size(index) 
...
count = zeros(length(index),1);
if iscell(sums) %sums is a cell array
    for i = 1:length(sums)
        count(cell2mat(ind(i)),1) = cell2mat(sums(i,1));
    end
else
    for i = 1:length(sums)
        count(cell2mat(ind(i)),1) = sums(i,1);
    end
end
