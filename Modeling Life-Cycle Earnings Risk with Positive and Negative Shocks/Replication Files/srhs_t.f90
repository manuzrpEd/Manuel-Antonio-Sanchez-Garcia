...
subroutine mexFunction(nlhs,plhs,nrhs,prhs)
    ...
    ! Function declarations
      ...
      ! Input           
      mwPointer  :: na,Np,Nn,NPP,nnodes,grid_n,c,grid_n_rho,c_p
...
!call computation function
	  call  srhs(%val(output),%val(na),%val(Np),%val(Nn),%val(NPP),%val(nnodes),%val(grid_n),%val(c) &
                 ,%val(grid_n_rho))
...
Subroutine srhs(output,na,Np,Nn,NPP,nnodes,grid_n,c,grid_n_rho)
...
real(8), dimension(na,Np,Nn,nnodes,NPP)                 :: c_pp
...
do j1 = 1,Np!%state
    do k1 = 1,nnodes
        do l1 = 1,na
            do m1 = 1,NPP!
		        call interp1D(c_pp(l1,j1,:,k1,m1),grid_n,c(l1,j1,:,k1,m1),grid_n_rho,Nn) 
            end do
        end do
    end do
end do

output = max(c_pp,0.d0)
