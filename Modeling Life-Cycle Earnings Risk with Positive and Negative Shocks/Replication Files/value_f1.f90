...
subroutine mexFunction(nlhs,plhs,nrhs,prhs)
    ...
    ! Function declarations
      ...
      ! Input           
      mwPointer  :: na,Np,Nn,NPP,nnodes,grid_p,grid_n,V_w,grid_p_rho,grid_n_rho,Eve_s
...
!call computation function
	  call  srhs_th(%val(output),%val(na),%val(Np),%val(Nn),%val(NPP),%val(nnodes),%val(grid_p),%val(grid_n) &
                 ,%val(V_w),%val(grid_p_rho),%val(grid_n_rho))
				       
      return  
      
    end subroutine
    
Subroutine srhs_th(output,na,Np,Nn,NPP,nnodes,grid_p,grid_n,V_w,grid_p_rho,grid_n_rho)

implicit none
integer(4), intent(in)                                  :: na,Np,Nn,NPP,nnodes
real(8), dimension(na,Np,Nn,nnodes,NPP),intent(in)      :: V_w
...
do i1 = 1,Np !%state
    do j1 = 1,Nn !%state
        do l1 = 1,na
            do k1 = 1,nnodes
                do m1 = 1,NPP
                    call interp2D(output(l1,i1,j1,k1,m1),grid_p,grid_n,V_w(l1,:,:,k1,m1),grid_p_rho(i1),grid_n_rho(j1),Np,Nn)
                end do
            end do
        end do
    end do
