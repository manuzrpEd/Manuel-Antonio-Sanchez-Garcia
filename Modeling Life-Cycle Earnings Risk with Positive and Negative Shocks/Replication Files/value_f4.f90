...
subroutine mexFunction(nlhs,plhs,nrhs,prhs)
    ...
    ! Function declarations
      ...
      ! Input           
      mwPointer  :: na,Np,Nn,NPP,nnodes,P_p,EVE_ps,P_tpL
...
!call computation function
	  call  srhs_th(%val(output),%val(na),%val(Np),%val(Nn),%val(NPP),%val(nnodes),%val(P_p) &
                 ,%val(EVE_ps),%val(P_tpL))
				       
      return  
      
    end subroutine
    
Subroutine srhs_th(output,na,Np,Nn,NPP,nnodes,P_p,EVE_ps,P_tpL)

implicit none
integer(4), intent(in)                                  :: na,Np,Nn,NPP,nnodes
real(8), dimension(Np,Np),intent(in)                    :: P_p
real(8), dimension(na,Np,Nn,nnodes,NPP), intent(in)     :: EVE_ps
...
do i1 =1,Np !state
    do j1 = 1,Np !transition to
        do k1 = 1,NPP
            output(:,i1,:,k1) = output(:,i1,:,k1)+P_p(i1,j1)*sum((EVE_ps(:,j1,:,:,k1)*P_tpL),3)
        end do
    end do
end do
