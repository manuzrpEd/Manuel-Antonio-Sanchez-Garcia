...
!--------------------------------------------------------------------------------    
    ...
    ! Retrieve input parameter pointers
     ...
     !plhs(1) = mxCreateNumericArray(ndim, dims, classid, complexflag)
      plhs(1) = mxCreateDoubleMatrix(mrows,8,0)
...
real(8), dimension(ro,8), intent(in)            :: resid_coh_x_drop
real(8), dimension(r,2), intent(in)              :: inte
real(8), dimension(r,8), intent(out)            :: nresid_coh_x_drop
...
do j1=1,r
    do i1=1,ro
        if (resid_coh_x_drop(i1,7)==inte(j1,1).AND.resid_coh_x_drop(i1,8)==inte(j1,2)) then
            !do t1=1,8
