function error_total=loss_simplified_notrans(para,data,H,T,randn_b,rand_shock_s,col,randn_a0)
clear mex
rand('seed',256344);
randn('seed',66573);
rho_n=para(1);
rho_p=para(2);
% sigma_b = para(3);
d_one_neg = para(3); %c_px - NEGAtive
d_two_neg = para(4); %b_px
d_one_pos = para(5); %d_one_pos - POSITIVE MARGINAL
d_two_pos = para(6); %d_two_pos
% sigma_ep0 = linspace(para(8),para(8)+para(9)*T,T)';
% sigma_en0 = linspace(para(10),para(10)+para(11)*T,T)';
sigma_x0 = linspace(para(7),para(7)+para(8)*T,T)';
sigma_z0 = linspace(para(9),para(9)+para(10)*T,T)';
mu_logna=linspace(para(11),para(11)+para(12)*T,T)';%positive
mu_lognb=linspace(para(13),para(13)+para(14)*T,T)';%negative
d_three_neg=para(15);%d_three_neg - NEGAtive
d_three_pos=para(16);
sigma_tr0 = linspace(para(17),para(17),T)';
sigma_a=para(18);%to be changed
sigma_epini=para(19);%0.5
sigma_enini=para(20);
mu_epini=para(21);%0.5
mu_enini=para(22);
...
panel_new(:,1:7) = moments_mixture(int32(H),int32(T),rho_n,rho_p,0,d_one_neg,d_two_neg,d_one_pos,d_two_pos,randn_ini_x,randn_ini_z,randn_b,randn_shock_ep,randn_shock_en ...
...
error_total=error_uncmean+error_mean_learn_age+error_ineq+error_outbound+ +error_var_p+error_var_n+error_kurt_cs+error_unc_acv+error_fract_p+error_skew_kelly+error_mean_p+error_mean_n;