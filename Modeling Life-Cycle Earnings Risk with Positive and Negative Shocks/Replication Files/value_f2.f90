...
subroutine mexFunction(nlhs,plhs,nrhs,prhs)
    ...
    ! Function declarations
      ...
      ! Input           
      mwPointer  :: na,Np,Nn,NPP,nnodes,grid_n,V_w,grid_n_rho,Eve_ps
...
! Retrieve input parameter pointers
      ...
      grid_n = mxGetPr(prhs(6))
      V_w = mxGetPr(prhs(7))
      grid_n_rho = mxGetPr(prhs(8))
...
!call computation function
	  call  srhs_th(%val(output),%val(na),%val(Np),%val(Nn),%val(NPP),%val(nnodes),%val(grid_n) &
                 ,%val(V_w),%val(grid_n_rho))
				       
      return  
      
    end subroutine
    
Subroutine srhs_th(output,na,Np,Nn,NPP,nnodes,grid_n,V_w,grid_n_rho)
...
do i1 = 1,Np !%state
        do l1 = 1,na
            do k1 = 1,nnodes
                do m1 = 1,NPP
                    call interp1D(output(l1,i1,:,k1,m1),grid_n,V_w(l1,i1,:,k1,m1),grid_n_rho,Np,Nn)
                end do
            end do
        end do
end do
