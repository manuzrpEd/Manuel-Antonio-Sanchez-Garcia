...
!size of output vector
     ...
     unc_var = mxGetPr(plhs(1))

!call computation function
	  call  Moment(%val(unc_var),%val(H),%val(T),%val(panel_new),%val(col),%val(cols),%val(col_g))
						      
      return  
      
    end subroutine
    
Subroutine Moment(unc_var,H,T,panel_new,col,cols,col_g)
implicit none

integer(4), intent(in)                           :: H,T,col,cols,col_g
real(8), dimension((T-1)*H,cols), intent(in)       :: panel_new
real(8), dimension(col), intent(out)             :: unc_var
