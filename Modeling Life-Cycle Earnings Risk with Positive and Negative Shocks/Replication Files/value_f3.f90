...
subroutine mexFunction(nlhs,plhs,nrhs,prhs)
    ...
    ! Function declarations
      ...
      ! Input           
      mwPointer  :: na,Np,Nn,NPP,nnodes,grid_p,V_w,grid_p_rho,Eve_ns
...
!call computation function
	  call  srhs_th(%val(output),%val(na),%val(Np),%val(Nn),%val(NPP),%val(nnodes),%val(grid_p) &
                 ,%val(V_w),%val(grid_p_rho))
				       
      return  
      
    end subroutine
    
Subroutine srhs_th(output,na,Np,Nn,NPP,nnodes,grid_p,V_w,grid_p_rho)
...
do i1 = 1,Nn !%state
        do l1 = 1,na
            do k1 = 1,nnodes
                do m1 = 1,NPP
                    call interp1D(output(l1,:,i1,k1,m1),grid_p,V_w(l1,:,i1,k1,m1),grid_p_rho,Np,Nn)
                end do
            end do
        end do
end do
