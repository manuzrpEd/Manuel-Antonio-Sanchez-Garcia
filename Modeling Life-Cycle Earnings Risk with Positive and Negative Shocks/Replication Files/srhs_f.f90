...
subroutine mexFunction(nlhs,plhs,nrhs,prhs)
    ...
    ! Function declarations
      ...
      ! Input           
      mwPointer  :: na,Np,Nn,NPP,nnodes,c_p,P_p,P_tpL,c_pp
    ...
    ! Array size information
      ...
      mwSize     :: ndim
      mwSize     :: dims(4)                    ! dimensions (for mxCreateNumericArray)
!--------------------------------------------------------------------------------    
...
!call computation function
	  call  srhs_th(%val(output),%val(na),%val(Np),%val(Nn),%val(NPP),%val(nnodes),%val(c_p),%val(P_p) &
                 ,%val(P_tpL),%val(c_pp))
...
Subroutine srhs_th(output,na,Np,Nn,NPP,nnodes,c_p,P_p,P_tpL,c_pp)

implicit none
integer(4), intent(in)                                  :: na,Np,Nn,NPP,nnodes
real(8), dimension(na,Np,Nn,nnodes,NPP),intent(in)      :: c_p
real(8), dimension(na,Np,Nn,NPP),intent(in)             :: c_pp
real(8), dimension(Np,Np), intent(in)                   :: P_p
real(8), dimension(na,Nn,nnodes), intent(in)            :: P_tpL
...
do i1 =1,Np !state
    do j1 = 1,Np !transition to
        do k1 = 1,NPP
            output(:,i1,:,k1) = output(:,i1,:,k1)+P_p(i1,j1)*sum((c_p(:,j1,:,:,k1)*P_tpL),3)
        end do
    end do
end do
