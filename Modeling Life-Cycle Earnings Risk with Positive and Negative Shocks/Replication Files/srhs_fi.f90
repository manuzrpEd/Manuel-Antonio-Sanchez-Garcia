...
subroutine mexFunction(nlhs,plhs,nrhs,prhs)
    ...
    ! Function declarations
      ...
      ! Input           
      mwPointer  :: na,Np,Nn,NPP,nnodes,c_n,P_n,P_tnL,c_nn
...
!call computation function
	  call  srhs_th(%val(output),%val(na),%val(Np),%val(Nn),%val(NPP),%val(nnodes),%val(c_n),%val(P_n) &
                 ,%val(P_tnL))
...
Subroutine srhs_th(output,na,Np,Nn,NPP,nnodes,c_n,P_n,P_tnL)

implicit none
integer(4), intent(in)                                  :: na,Np,Nn,NPP,nnodes
real(8), dimension(na,Np,Nn,nnodes,NPP),intent(in)      :: c_n
...
do i1 =1,Nn !state
    do j1 = 1,Nn !transition to
        do k1 = 1,NPP
            output(:,:,i1,k1) = output(:,:,i1,k1)+P_n(i1,j1)*sum(c_n(:,:,j1,:,k1)*P_tnL,3)
        end do
    end do
end do
