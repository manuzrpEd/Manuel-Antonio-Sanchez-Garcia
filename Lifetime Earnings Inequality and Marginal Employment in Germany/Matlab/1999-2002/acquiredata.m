...
npeople=max(data_persnr_males(:,15));
...
for i=1:size(empstate,1)
    for j=1:size(empstate,2)
        if empstate(i,j)==5
            empstate(i,j)=1;
        end
    end
...
lmexp=(cumsum(ones(npeople,nobs_months),2)-1)/10;%labor market experience I think this is wrong, because I have attrition. 
% Should use age instead in Estimation.
...
data_male.lFT=lFT(:,1:nobs_months-1);
data_male.lPT=lPT(:,1:nobs_months-1);
data_male.lMJ=lMJ(:,1:nobs_months-1);
data_male.lNE=lNE(:,1:nobs_months-1);
...
data_female.lFT=lFT(:,1:nobs_months-1);
data_female.lPT=lPT(:,1:nobs_months-1);
data_female.lMJ=lMJ(:,1:nobs_months-1);
data_female.lNE=lNE(:,1:nobs_months-1);
